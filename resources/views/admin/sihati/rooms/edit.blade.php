...
<div class="container-fluid py-4">
    ...
    <div class="d-flex align-items-center gap-3 mb-4">
        <a href="{{ route('admin.sihati.rooms.show', $room) }}" class="btn btn-outline-secondary btn-sm">
            <i class="bi bi-arrow-right"></i>
        </a>
        <h4 class="mb-0 fw-bold">تعديل غرفة: {{ $room->name }}</h4>
    </div>
                    ...
                    <form action="{{ route('admin.sihati.rooms.update', $room) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="mb-3">
                            ...
                            <select name="patient_id" class="form-select @error('patient_id') is-invalid @enderror">
                                ...
                                @foreach($patients as $patient)
                                    <option value="{{ $patient->id }}" {{ old('patient_id', $room->patient_id) == $patient->id ? 'selected' : '' }}>
                        ...
                        <div class="mb-3">
                            ...
                            <select name="created_by" class="form-select @error('created_by') is-invalid @enderror">
                                ...
                                @foreach($superNurses as $superNurse)
                                    <option value="{{ $superNurse->id }}" {{ old('created_by', $room->created_by) == $superNurse->id ? 'selected' : '' }}>
                                        {{ $superNurse->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-3">
                            ...
                            <textarea name="description" rows="3"
                                ...
                                placeholder="وصف اختياري للغرفة">{{ old('description', $room->description) }}</textarea>
                        ...
                        <div class="mb-4">
                            ...
                            <select name="registration_template_id" class="form-select @error('registration_template_id') is-invalid @enderror">
                                ...
                                @foreach($registrationTemplates as $template)
                                    <option value="{{ $template->id }}" {{ old('registration_template_id', $room->registration_template_id) == $template->id ? 'selected' : '' }}>
                                        {{ $template->name }}
                                    </option>
                                @endforeach
                            </select>

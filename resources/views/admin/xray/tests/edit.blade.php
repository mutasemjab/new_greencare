...
@section('title', 'تعديل الأشعة')

@section('content')
<div class="container-fluid py-4">

    <div class="d-flex align-items-center gap-3 mb-4">
        <a href="{{ route('admin.xray.tests') }}" class="btn btn-outline-secondary btn-sm">
            <i class="bi bi-arrow-right"></i>
        </a>
        <h4 class="mb-0 fw-bold">تعديل الأشعة: {{ $test->name }}</h4>
    </div>
    ...
    <div class="card border-0 shadow-sm mx-auto" style="max-width:700px;">
        <div class="card-body p-4">
            <form action="{{ route('admin.xray.tests.update', $test) }}" method="POST">
                @csrf
                @method('PATCH')

                <div class="mb-3">
                    <label class="form-label fw-semibold">الفئة <span class="text-danger">*</span></label>
                    <select name="xray_category_id" class="form-select @error('xray_category_id') is-invalid @enderror" required>
                        <option value="">-- اختر الفئة --</option>
                        @foreach($categories as $cat)
                            <option value="{{ $cat->id }}"
                                @selected(old('xray_category_id', $test->xray_category_id) == $cat->id)>
                                {{ $cat->name }}
                            </option>
                        @endforeach
                    </select>
                    @error('xray_category_id')
                ...
                <div class="mb-4">
                    <div class="form-check form-switch">
                        <input class="form-check-input" type="checkbox" name="is_active" id="is_active"
                            value="1" {{ old('is_active', $test->is_active) ? 'checked' : '' }}>
                ...
                <div class="d-flex gap-2">
                    ...
                    <a href="{{ route('admin.xray.tests') }}" class="btn btn-outline-secondary px-4">إلغاء</a>
                </div>

...
@section('title', 'طلبات الاستحمام')

@section('content')
<div class="container-fluid py-4">

    <div class="d-flex align-items-center justify-content-between mb-4">
        <h4 class="mb-0 fw-bold">طلبات الاستحمام</h4>
    </div>
    ...
    <div class="card border-0 shadow-sm mb-4">
        <div class="card-body">
            <form method="GET" action="{{ route('admin.bathing.requests') }}" class="row g-3">
                <div class="col-md-3">
                ...
                <div class="col-md-3">
                    <select name="payment_type" class="form-select">
                        ...
                        <option value="card" @selected(request('payment_type') === 'card')>بطاقة</option>
                        <option value="cash" @selected(request('payment_type') === 'cash')>نقدي</option>
                    </select>
                </div>
                <div class="col-md-3 d-flex gap-2">
                    ...
                    <a href="{{ route('admin.bathing.requests') }}" class="btn btn-outline-secondary w-100">
                        ...
                        <tr>
                            ...
                            <td>
                                @if($req->payment_type === 'card')
                                    <span class="badge bg-info-subtle text-info">بطاقة</span>
                                @elseif($req->payment_type === 'cash')
                                    <span class="badge bg-warning-subtle text-warning">نقدي</span>
                                @else
                            ...
                            <td>
                                @if($req->card)
                                    <code class="small">{{ $req->card->code }}</code>
                                @else
                                    <span class="text-muted">—</span>
                                @endif
                            </td>
                            <td>{{ $req->soldAtPoint?->name ?? '—' }}</td>
                            <td>{{ $req->booking_date ? \Carbon\Carbon::parse($req->booking_date)->format('Y/m/d') : '—' }}</td>
                            <td>
                                <span class="badge {{ $st['class'] }}">{{ $st['label'] }}</span>
                            </td>
                            <td>
                                <a href="{{ route('admin.bathing.requests.show', $req) }}"

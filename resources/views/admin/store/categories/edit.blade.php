...
@section('title', 'تعديل تصنيف المتجر')
...
<div class="container-fluid py-4">
    ...
    <div class="d-flex align-items-center gap-3 mb-4">
        ...
        <h4 class="mb-0 fw-bold">تعديل التصنيف: {{ $storeCategory->name }}</h4>
    </div>
    ...
    <div class="card border-0 shadow-sm mx-auto" style="max-width:700px;">
        <div class="card-body p-4">
            <form action="{{ route('admin.store.categories.update', $storeCategory) }}" method="POST"
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="name" class="form-control @error('name') is-invalid @enderror"
                        value="{{ old('name', $storeCategory->name) }}" required>
                    @error('name')
                    ...
                    <select name="parent_id" class="form-select @error('parent_id') is-invalid @enderror">
                        ...
                        @foreach($parents as $parent)
                            <option value="{{ $parent->id }}"
                                @selected(old('parent_id', $storeCategory->parent_id) == $parent->id)>
                                {{ $parent->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-3">
                    ...
                    <div class="d-flex align-items-center gap-3">
                        ...
                        <div class="form-check">
                            ...
                            <label class="form-check-label text-danger" for="remove_image">حذف الصورة</label>
                        </div>
                    </div>
                </div>
                @endif
                ...
                {{-- New Image --}}
                <div class="mb-3">
                    <label class="form-label fw-semibold">{{ $storeCategory->image ? 'تغيير الصورة' : 'الصورة' }}</label>
                ...
                <div class="mb-3">
                    ...
                    <input type="number" name="sort_order" class="form-control @error('sort_order') is-invalid @enderror"
                        value="{{ old('sort_order', $storeCategory->sort_order) }}" min="0">
                    @error('sort_order')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="mb-4">
                    <div class="form-check form-switch">
                        <input class="form-check-input" type="checkbox" name="is_active" id="is_active"
                            value="1" {{ old('is_active', $storeCategory->is_active) ? 'checked' : '' }}>
